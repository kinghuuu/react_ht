...
//添加外部dblink弹框
class OutDBLinkModal extends Component {
    constructor(props) {
        super(props);
        this.state = {
            outdblinkTabList: [
    ...
    componentDidMount() {
        const { outdblinkRecord: { dbInfo = '' } } = this.props;
    ...
    handleSubmit = () => {
        let { form: { validateFields }, outdblinkRecord, parentRecord, dispatch } = this.props;
        validateFields((error, fieldsValue) => {
            ...
            if (outdblinkRecord.nodeId) {
                params.nodeId = outdblinkRecord.nodeId
    ...
    addIP = (key) => {
        ...
        const { outdblinkTabList } = this.state;
        let tablist = _.cloneDeep(outdblinkTabList);
        ...
        authKeys.push('');
        this.setState(() => ({ outdblinkTabList: tablist }), () => {
            setFieldsValue({
                table: tabKeys,
                permission: authKeys
            });
    ...
    render() {
        const { form: { getFieldDecorator }, outdblinkRecord, isLook, visible } = this.props;
        const { userName = '', dblinkName, dblinkSystem } = outdblinkRecord;
        let { outdblinkTabList } = this.state;
        const formItemLayout2 = {
            labelCol: { span: 8 },
            wrapperCol: { span: 15 }
        };
        const IPListInfo = _.map(outdblinkTabList, (item, index) => (
        ...
        return (
            <Modal
                title={outdblinkRecord.nodeId ? '修改dblink' : '新增dblink'}
                visible={visible}
                okText='保存'
                maskClosable={false}
                destroyOnClose={true}
                ...
                <Form>
                    <FormItem {...formItemLayout} label='外部分配的用户名'>
                        {
                            getFieldDecorator('userNamedblink', {
                                rules: [{ required: true, message: '外部分配的用户名不能为空' }],
                                initialValue: userName
                            })
                                (<Input autoComplete='off' placeholder='请输入外部分配的用户名' />)}
                    ...
                    <FormItem  {...formItemLayout} label='外部开放的系统'>
                        {getFieldDecorator('dblinkSystem', {
                            initialValue: dblinkSystem
                        })(<Input autoComplete='off' placeholder={isLook ? '' : '请输入外部开放的系统'} />)}
                    </FormItem>
                    {IPListInfo}
...
OutDBLinkModal = Form.create()(OutDBLinkModal);

export default connect()(OutDBLinkModal);