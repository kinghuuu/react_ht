...
import DBLinkModal from './dblinkModal';
...
class DBLink extends Component {
    ...
    delRow = (record, e) => {
        ...
        if (record.nodeId) {
            ...
            if (_.isEmpty(selectedRows)) {
                message.warning('请先选择待删除的dblink！');
                return;
            }
            confirm({
                title: '删除dblink',
                content: `确定删除选中的${selectedRows.length}个dblink?`,
                onOk: () => {
        ...
        validateFields((err, fieldsValue) => {
            var params = {
                ...
                propertyValueDtoList: [
                    {
                        propId: "userName",
                        propValue: fieldsValue.queryuserName || ''
                    },
                    {
                        propId: "dblinkName",
                        propValue: fieldsValue.querydblinkName || ''
                    }
                ],
            }
    ...
    render() {
        ...
        const uploadProps = {
            data: {
                systemId, parentId: record.nodeId, configItemType: 'oracleInnerType'
            },
            name: 'file',
        ...
        const columns = [
            ...
            { title: '使用系统', dataIndex: 'dblinkSystem', width: '20%' },
            {
                title: '开放表名及权限', dataIndex: 'dbInfo', width: '25%',
                render: (tab, record) => {
                        ...
                        <Col span={24} style={{ marginLeft: 8, lineHeight: '38px' }}>
                            ...
                            <Upload {...uploadProps}>
                                <Button
                                    type='primary'
                                    icon='import'
                                    style={{ backgroundColor: '#f0ad4e', borderColor: '#eea236' }}>导入dblink</Button>
                            </Upload>
                            <Button
                                type='primary'
                                icon='download'
                                onClick={() => window.open(`${ContextPath}/cmdbCommon/downloadTemp?configItemType=oracleInnerType`)}
                    ...
                    <Row className={styles.queryCol}>
                        <Col span={7}>
                            <FormItem {...formLayout} label='用户名'>
                                {
                                    getFieldDecorator('queryuserName')
                        ...
                        <Col span={7}>
                            <FormItem {...formLayout} label='dblink名称'>
                                {
                                    getFieldDecorator('querydblinkName')
...
DBLink = Form.create()(DBLink)
export default connect()(DBLink);
