...
class ServiceOperation extends Component {
    constructor(props) {
        super(props);
    ...
    handleSearch = (e) => {
        ...
        let { serverName = '', processId: { value: [processId = ''] } } = getFieldsValue();
        if (isShowOtherQuery) {
            selfDefineQuery = this.searchForm.getValue();
        }
        propertyValue = [
            { propId: 'serverName', propValue: serverName },
            { propId: 'processId', propValue: processId },
        ]
                                ...
                                getFieldDecorator('processId', {
                                    ...
                                    (
                                        <SearchInput
                                            ...
                                            placeholder={`请选择所属进程`}
                                            method='get'
                                            attachParams={{ systemId, nodeType: 'process' }}
...
ServiceOperation = Form.create()(ServiceOperation);

export default connect(mapStateToProps)(ServiceOperation);