...
class ServiceSliOperation extends Component {
    constructor(props) {
        super(props);
    ...
    handleSearch = (e) => {
        ...
        let { serviceId: { value: [serviceId = ''] } } = getFieldsValue();

        if (isShowOtherQuery) {
            selfDefineQuery = this.searchForm.getValue();
        }
        propertyValue = [
            { propId: 'serviceId', propValue: serviceId },
        ]
...
ServiceSliOperation = Form.create()(ServiceSliOperation);
export default connect(mapStateToProps)(ServiceSliOperation);